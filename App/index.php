...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'POST':
        ...
        if ($success) {
            ...
            echo json_encode(["message" => "Dados cadastrados com sucesso."]);
        } else {
            http_response_code(500); // Internal Server Error
            echo json_encode(["message" => "Falha ao cadastrar dados."]);
        }
        break;
    ...
    case 'GET':
        try{
            ...
            if (isset($_GET['id'])) {
                ...
                if ($id === false) {
                    http_response_code(400); // Bad Request
                    echo json_encode(['error' => 'O valor do ID fornecido não é válido.']);
                    exit;
                } else {
